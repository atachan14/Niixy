...
公開 AccountPage と、Account が作成または返信した Thread の一覧・閲覧導線を追加する。Guest を含む利用者が Account を確認し、その Account の公開範囲にある Thread を SummaryList / DetailPane の共通パターンで閲覧・返信できる状態をつくる。
...
- Account が作成した Thread と返信した Thread を ThreadPane で表示する。
...
- `返信`: 対象 Account が `#2` 以降の Response を投稿した Thread。同一 Thread への複数返信は一件にまとめ、対象 Account による最新 Response が新しい順。
...
- 作成 Thread と返信 Thread を条件どおり重複なく表示できる。

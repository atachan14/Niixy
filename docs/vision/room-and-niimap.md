...
現行の NiiMap は地図に直接掲載された Thread を表示する。将来は、Room 側の制限が発見を許可する場合に Room から共有された Thread も表示できるようにする。Tweet、Book、Book に配置された Thread など、複数種別のコンテンツを表示対象へ加える可能性がある。
...
Thread は一つの主掲載先を持ち、追加の共有先を持てる。NiiMap、Room、Book は掲載先または共有先になり得る。移動は主掲載先の変更、共有は掲載先の追加として扱う。両者は履歴上で区別でき、Thread 自体を複製しない。
